...
**Extraction precision/recall (measured separately from end-to-end correctness):** PLANNING.md's Evaluation Metrics section names extraction precision/recall as its own metric — a wrong final answer elsewhere in the pipeline (resolution, retrieval) could otherwise be mistaken for an extraction problem or vice versa. A labeled 20-case ground-truth set (`eval/scenarios/extraction_cases.jsonl`, matching by extracted value rather than exact relation string, since relation-naming reasonably varies) was run live against the real extractor: **100% precision, 100% recall (15/15 expected facts, 0 false positives, 0 false negatives)**. Read that precisely: it means the extractor made zero mistakes on these 20 specific cases, not that it is error-free in general — the set is intentionally small (a supplementary diagnostic metric, not the core 50-scenario suite), and the occasional malformed-JSON extraction failures noted below are a separate, real failure mode this metric doesn't capture (it measures decision/value correctness on cases that returned parseable output).
